...
public class ScheduleGenerator {
	...
	private void draw(Graphics2D graphics, CoordinateTranslator ct, List<DayRow> dayRows) {
		...
		for (int i=0; i<cnt; i++) {
			graphics.drawLine(ct.encodeX(0), ct.encodeY(perLine*i),  
					ct.encodeX(1.0), ct.encodeY(perLine*i));
			graphics.drawLine(ct.encodeX(0), ct.encodeY(perLine*(i+1)),  
					ct.encodeX(1.0), ct.encodeY(perLine*(i+1)));
		}
		...
		for (int i=0; i<cnt; i++) {
			...
			for (Timetable t : dr.ordered) {
				...
				graphics.drawLine(ct.encodeX(0), ct.encodeY(0),  
						ct.encodeX(0), ct.encodeY(1.0));
				...
				graphics.drawLine(ct.encodeX(1.0), ct.encodeY(0),  
						ct.encodeX(1.0), ct.encodeY(1.0));
			...
			for (DayRow dr : drl) {
				if (dr.day == day) {
					...
					for (Timetable tt : dr.ordered) {
						if ((tt.getClassFrom().before(t.getClassTo()))
								&& (tt.getClassTo().after(t.getClassFrom()))) {
							conflicts = true;
							break;
						}						
					}
